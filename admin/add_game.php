<?php
    ...
    {
        if(isset($_POST['subsubject_id']) && isset($_POST['game_description']))
        {
            include "../db.php";
			if($stmt = $mysqli->prepare("INSERT INTO game(subsubject_id, description) values(?, ?)"))
            {
                $stmt->bind_param("is",$_POST['subsubject_id'], $_POST['game_description']);
                $stmt->execute();
    ...
    <body>
        <?php
            if($notice != null)
                echo $notice . "<br />";
        ?>
        <form action="add_game.php" method="post" name="add_game">
            Хичээл:
            ...
            Дадлага:
            <input type="text" name="game_description" /><br />
